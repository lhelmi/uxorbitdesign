    ...
    <div class="col-12 col-xl-9 col-lg-9">
        ...
        <div class="c-stage__header o-media u-justify-start cursor-default <?php echo ($this->input->get('editcourse') == 'false') ? 'u-hidden' : '' ?>">
                <div class="c-stage__header-title o-media__body">
                    ...
                    <?php endif ?>
                    <button class="c-btn c-btn--info c-btn--custom" name="publish" type="submit" title="kirim">
                        <i class="fa fa-paper-plane" aria-hidden="true"></i> Kirim
                    </button>
                    <?php if (!empty($data)) : ?>
                        <button class="u-ml-small c-btn c-btn--primary c-btn--custom" type="submit" name="save" title="simpan" value="<?php echo uri_string(); ?>">
                ...
                <div class="u-ml-auto" style="min-width: 150px">
                    <?php if (!empty($data['is_new'])): ?>
                    <div class="form-group" id="verived">
                        <select name="is_new" id="is_new" class="form-control">
                            <option value=""></option>
                            <option <?= $data['is_new'] == 'Terverifikasi' ? 'selected' : '' ?> value="Terverifikasi">Terverifikasi</option>
                            <option <?= $data['is_new'] == 'Belum Verifikasi' ? 'selected' : '' ?>  value="Belum Verifikasi">Belum Verifikasi</option>
                        </select>
                    </div>
                    <?php endif ?>
                    <div class="c-toggle">
                ...
                <?php }elseif(!empty($data['status']) && $this->session->userdata('app_grade') == 'Instructor' && $data['is_new'] == 'Terverifikasi'){ ?>
                    <div class="u-ml-auto" style="min-width: 150px">
                        <div class="c-toggle">
                            <div class="c-toggle__btn <?php echo (!empty($data['status'])) ? ($data['status'] == 'Published') ? 'is-active' : '' : ''?>">
                                <label class="c-toggle__label" for="publish">
                ...
                <div class="c-stage u-mb-zero u-border-zero" id="accordion">
                    ...
                    <div data-parent="#accordion" class="c-stage__panel u-p-xsmall collapse show" id="stage-image">
                        ...
                        <div class="c-field has-addon-right">
                            <input type="file" <?php echo (!empty($data['image']) ? '' : 'required') ?> name="image" id="image" onchange="showImage(this);" value="<?php echo (!empty($data['image']) ? $data['image'] : '') ?>">
