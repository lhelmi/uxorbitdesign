...
class M_Courses extends CI_Model
{
	...
	public function data_course($site, $slug)
	{
		...
		return array_merge($courses, $user_courses, $build_lesson, $related_post, $review, $review_pagination, $review_stats);
	}
	...
	function get_bough($id)
	{	
		$this->db->select('count(status) as bough');
		// $this->db->join('id_courses', );
	...
	{	
		...
		// $this->db->join('id_courses', );
		$this->db->where('id_courses', $id);
    	return $this->db->get('tb_lms_user_courses')->row_array();
    }
}
