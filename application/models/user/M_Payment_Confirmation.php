...
class M_Payment_Confirmation extends CI_Model
{
    ...
    public function confirm() {
        $this->db->select('*');
        $this->db->join('tb_user', 'tb_user_contacts.user_id = tb_user.id');
        $this->db->where('tb_user_contacts.user_id', '1');
    ...
    public function set_validation(){
        ...
        $this->form_validation->set_rules('sender', 'lang:proof_sender', 'required|min_length[3]|max_length[100]trim', [
            'required' => '{field} '.$this->lang->line('must_filled'),
            ]);
    }        
