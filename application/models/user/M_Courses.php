...
class M_Courses extends CI_Model
{
	...
	{
		$this->db->select('tb_lms_user_payment.id as id_payment, tb_lms_courses.*, tb_lms_user_courses.time as waktu,
		tb_user.nama_lengkap');
		$this->db->join('tb_lms_user_payment', 'tb_lms_user_payment.id_courses = tb_lms_courses.id');
		$this->db->join('tb_user', 'tb_lms_courses.id_user = tb_user.id');
		$this->db->join('tb_lms_user_courses', 'tb_lms_user_courses.id_courses = tb_lms_courses.id');
	...
	public function process_quiz(){
		...
		if ($user_lesson->num_rows() < 1) {
			...
			$this->_Process_MYSQL->update_data($this->table_lms_user_lesson,$post_data,[
				'id_user' => $this->session->userdata('id_user'),
				'id_courses' => $id_courses,
			]);
		}
