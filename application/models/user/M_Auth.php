...
class M_Auth extends CI_Model
{
                ...
                if ($this->_Process_MYSQL->update_data($this->table_user, $data_update, array('id' => $id)) == TRUE) {
                    ...
                    if ($read_data['grade'] == 'User') {
                        $this->session->set_userdata(array(
                            ...
                            'nama_lengkap' => $read_data['nama_lengkap'],                        
                            'user' => "login"
                            ));
        ...
        if ($post_data['csrf_code'] != '' AND $this->session->userdata('csrf_code') == $post_data['csrf_code']) {
            ...
            if ($email_vertification == 'Yes') {
                $config = [
                    ...
                    'smtp_user' => 'geraldkedua2@gmail.com',
                    'smtp_pass'   => 'G4h0pk4c4',
                    'smtp_crypto' => 'ssl',
                    'smtp_port'   => 465,
                    'crlf'    => "\r\n",
                ...
                $this->email->from('no-reply@UXOrbit', 'UXOrbit');
    ...
    public function RegisterSocialMedia($data){
        ...
        if ($process) {
            $this->session->set_userdata(array(
                'nama_lengkap' => $process['nama_lengkap'],                        
                'id_user' => $process,                        
    ...
    function logout(){
        ...
        $this->session->unset_userdata(['id','app_username','app_photo','app_grade','status','key', 'nama_lengkap']);
        $this->session->unset_userdata(['id_user','username','photo','grade','user', 'nama_lengkap']);
