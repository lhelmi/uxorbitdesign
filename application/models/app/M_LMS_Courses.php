...
class M_LMS_Courses extends CI_Model
{
    ...
    public function data_table()
    {
        ...
        $this->datatables->edit_column('title', '
            ...
            ', 'ctsubstr(title,60),permalink,formatstatus(timeorigin,status),views, is_new');
        ...
        $this->datatables->add_column('alat', '
            ...
            <a class="c-btn--custom c-btn--small c-btn c-btn--info" href="' . base_url('app/lms_courses/') . 'update/$1"><i class="fa fa-edit"></i>Edit</a>

            <button type="button" data-title="Beneran mau dihapus ?" data-text="Mau menghapus $2" class="c-btn c-btn--danger c-btn--custom action-delete" data-href="' . base_url('app/lms_courses/delete/$1') . '">
            <i class="fa fa-trash"></i> Hapus
            </button>
            ', 'id,title');

        $this->datatables->add_column('view', '
          <button type="button" class="c-btn--custom c-btn--small c-btn c-btn--primary" name="action-view"><i class="fa fa-eye"></i>Detail</button>
    ...
    public function create_data_post()
    {
            ...
            $config['upload_path'] = './storage/uploads/medium/images/';
            $config['allowed_types'] = 'jpg|png|PNG|jpeg|gif';
            $config['max_size']  = '5048';
            $config['overwrite'] = true;
            $new_name = 'images/'.strtolower(slug($this->input->post('title'))).time().$_FILES["image"]['name'];
            $config['file_name'] = strtolower(slug($this->input->post('title'))).time().$_FILES["image"]['name'];
    ...
    public function data_post()
    {
        ...
        if($this->session->userdata('app_grade') == 'App' ){
            if (!empty($_FILES['image']['name'])) {
                ...
                $config['max_size']  = '5048';
                $config['overwrite'] = true;
                $spacing = str_replace(' ', '', $_FILES["image"]['name']);
                $new_name = time().$spacing;
    ...
    {
        return $this->_Process_MYSQL->insert_data($this->table_lms_courses, $this->create_data_post(), true);
    }
    ...
    function check_user(){
        ...
        $this->db->get('tb_lms_user_payment');
    }
    ...
    {
        $data = [
            'id_lms_courses_lesson' => $this->input->post('id_section'),
        ];
