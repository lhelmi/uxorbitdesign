...
class M_Setting_Payment extends CI_Model
{
	...
	public function read_comfirm(){

		$read = $this->_Process_MYSQL->get_data('tb_user_contacts',['id' => $this->session->userdata('id')])->row_array();
		$read = json_decode($read['payment'],true);	
	...
	{
		$this->db->join('tb_bank_account', 'tb_lms_user_payment.id_courses_user = tb_bank_account.id');
		$this->db->where('tb_lms_user_payment.id', $id);
		$this->db->where('tb_bank_account.id_user', '1');
    	return $this->db->get('tb_lms_user_payment')->result_array();
	}
	
	function get_bank($token)
	{
		$this->db->select('tb_bank_account.*');
		$this->db->where('tb_bank_account.type', $token);
		$this->db->where('tb_bank_account.id_user', '1');
	...
	{
		$this->db->select('tb_lms_user_payment.token');
		$this->db->where('id', $id);
	...
	public function process_confirm(){
		...
		$this->db->insert('tb_user_contacts', $insert);
	...
	public function process_create(){
		...
		$this->db->insert('tb_bank_account', $insert);
	...
	public function process_update_confirm(){
		$update = [
			'medium' => $this->input->post('medium'),
			'description' => $this->input->post('description'),
		];
		$this->db->where('id', $this->input->post('identity'));
		$this->db->where('user_id', $this->session->userdata('id'));
		$this->db->update('tb_user_contacts', $update);
	}	
	...
	public function process_update(){
		...
		$this->db->where('id', $this->input->post('identity'));
		$this->db->update('tb_bank_account', $update);
	}	
